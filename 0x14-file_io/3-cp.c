...
{
	int fd_to, fd_from, actual_read, actual_write, actual_close_to, actual_close_from;
	char *buff;
	...
	fd_to = open(file_to, O_CREAT | O_RDWR | O_TRUNC, (S_IRUSR | S_IWUSR | S_IRGRP
				| S_IWGRP | S_IROTH)); 
	if (fd_to == -1)
		return (-1);
	...
	actual_close_to = close(fd_to);
	actual_close_from = close(fd_from);
	if (actual_close_to == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't close fd %d\n", fd_to);
		exit(100);
	}
	if (actual_close_from == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't close fd %d\n", fd_from);
		exit(100);
	}
